...
if (argc >1) {
	...
	int num = MultiByteToWideChar(0,0,"winsta0\\default",-1,NULL,0);
	wchar_t *wide = new wchar_t[num];
	MultiByteToWideChar(0,0,"winsta0\\default",-1,wide,num);
	si.lpDesktop = wide;//????
	CreateProcessWithTokenW(hToken, LOGON_NETCREDENTIALS_ONLY, NULL, ToLPWSTR(argv[1]), NORMAL_PRIORITY_CLASS, NULL, NULL, &si, &pi);
	CloseHandle(hToken);
	return 0;
}
    ...
    do {
        ...
        switch (choice) {
            ...
            case 4:
                ...
                break;
            case 5:
            	printf("LXZY: ACG, optimize and bug fix.\n");
            	printf("YF: Author.");
            	Sleep(5000);
            	break;
            default:
